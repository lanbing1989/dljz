...
// 查询所有服务期
$periods = [];
$stmt1 = $db->prepare("SELECT * FROM service_periods WHERE contract_id=:id ORDER BY service_start DESC");
...
// 查询所有分段
$segments_by_period = [];
$stmt2 = $db->prepare("SELECT * FROM service_segments WHERE service_period_id IN (SELECT id FROM service_periods WHERE contract_id=:id)");
...
// 查询临时收费
$stmt3 = $db->prepare("SELECT * FROM payments WHERE contract_id=:id AND is_temp=1 ORDER BY pay_date DESC, id DESC");
$stmt3->bindValue(':id', $id, SQLITE3_INTEGER);
$temp_payments = $stmt3->execute();
?>
<!DOCTYPE html>
<html>
<head>
        ...
        <?php foreach($periods as $p): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="service_period_delete.php?id=<?=urlencode($p['id'])?>&contract_id=<?=urlencode($id)?>" class="btn btn-sm btn-danger" onclick="return confirm('确定要删除本服务期及其分段和收费记录吗？')">删除</a>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
