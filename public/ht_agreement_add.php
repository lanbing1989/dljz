...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 获取合同模板内容
    $stmt_template = $db->prepare("SELECT * FROM contract_templates WHERE id=:id");
    $stmt_template->bindValue(':id', $template_id, SQLITE3_INTEGER);
    $template = $stmt_template->execute()->fetchArray(SQLITE3_ASSOC);
    $template_content = $template ? $template['content'] : '';

    // 获取客户信息
    $stmt_client = $db->prepare("SELECT * FROM contracts WHERE id=:id");
    $stmt_client->bindValue(':id', $client_id, SQLITE3_INTEGER);
    $client = $stmt_client->execute()->fetchArray(SQLITE3_ASSOC);

    // 服务期
    $period = null;
    if ($service_period_id) {
        $stmt_period = $db->prepare("SELECT * FROM service_periods WHERE id=:id");
        $stmt_period->bindValue(':id', $service_period_id, SQLITE3_INTEGER);
    ...
    if ($service_segment_id) {
        ...
        $stmt_segment->bindValue(':id', $service_segment_id, SQLITE3_INTEGER);
    ...
    if ($seal_id) {
        ...
        $stmt_seal->bindValue(':id', $seal_id, SQLITE3_INTEGER);
